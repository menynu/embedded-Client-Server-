...
Accepted input:
	...
	4: As server, CIPMUX must be 1, so open server to multiply connections.
		Command executed: AT+CIPMUX=1
	5: listen to port 80 in server
		Command executed: AT+CIPSERVER=1,80
	6: set the TCP server timeout to infinity
		Command executed: AT+CIPSTO=0
	7: display how much client connected
		Command executed: AT+CIPSTATUS
	9: end the connection with the client
		Command executed: AT+CIPCLOSE=0
	L: activate infinity loop
******************************************************************************/
